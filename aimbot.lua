...
getgenv().ExunysDeveloperAimbot = {
	...
	Settings = {
		...
		Sensitivity2 = 0.85,            -- mousemoverel speed (0.6–1.3 range)
		SmoothingFactor = 0.35,         -- NEW: smooth controller (0.2 = super smooth/slow, 0.5 = snappy)

		LockPart = "Head",

		-- Trigger keys (BOTH work at the same time!)
		TriggerKey = Enum.UserInputType.MouseButton2,   -- Right click for KBM
		TriggerKeyController = Enum.KeyCode.ButtonL2,   -- L2 for controller (change to ButtonR2 if you prefer)
...
local GetConnectedGamepads = __index(UserInputService, "GetConnectedGamepads")  -- ADD THIS LINE near other service methods (e.g. after GetMouseLocation)

local GetClosestPlayer = function()
    local Settings = Environment.Settings
    local LockPart = Settings.LockPart
    -- NEW: Controller detection & lead time (more prediction for controller = better lock-on)
    local connectedPads = GetConnectedGamepads(UserInputService)
    local leadTime = (#connectedPads > 0) and 0.15 or 0.08  -- Controller: 150ms lead (smooth/sticky), KBM: 80ms (snappy)
    if not Environment.Locked then
        ...
        for _, Value in next, GetPlayers(Players) do
            ...
            and Humanoid then
                ...
                -- NEW: Velocity prediction (locks where target WILL BE, amazing for controller tracking)
                ...
                if RootPart then
                    ...
                    predictedPos = currentPos + vel * leadTime
                end
                ...
                if Settings.WallCheck then
                    ...
                    if #GetPartsObscuringTarget(Camera, {currentPos}, BlacklistTable) > 0 then continue end  -- Wallcheck CURRENT pos (predicted might clip)
                end
                local Vector, OnScreen = WorldToViewportPoint(Camera, predictedPos)  -- Screen dist from PREDICTED
	...
	ServiceConnections.RenderSteppedConnection = Connect(RunService[Environment.DeveloperSettings.UpdateMode], function()
		...
		if Running and Settings.Enabled then
			...
			if Environment.Locked then
				local LockedPosition_Vector3 = __index(__index(Environment.Locked, "Character")[LockPart], "Position") + Offset
				local LockedPosition = WorldToViewportPoint(Camera, LockedPosition_Vector3)

				if Settings.LockMode == 2 then
					-- SMOOTH mousemoverel (no more crazy jitter on controller)
					...
					local moveX = deltaX * Settings.Sensitivity2 * Settings.SmoothingFactor
					local moveY = deltaY * Settings.Sensitivity2 * Settings.SmoothingFactor
